...
error() {
echo ""
echo -e "${negro}[${verde} !! ${negro}] >>>${verde} Servicio de ayuda +52 22833821202${negro} :)"
echo -e "${negro}[${verde} :D ${negro}] >>>${verde} JoséDíaz el mejor!!${negro} :)"
echo -e "${negro}[${verde} !! ${negro}] >>>${verde} Proceso interrupido por el usuario${negro} :) $*"; echo "" ;exit 1
}
...
sleep 2
echo ""
echo -e "${negro}[${verde} :D ${negro}] >>>${verde} Elige un puerto${negro} :)\n"
echo -e -n "${negro}[${verde} :D ${negro}] >>>" ${verde}
read -r port
cd sites/pagina && php -S 127.0.0.1:${port} > /dev/null 2>&1 &
sleep 2
echo ""
echo -e "${negro}[${verde} !! ${negro}] >>>${verde} LINK : localhost:${port}"
echo ""
